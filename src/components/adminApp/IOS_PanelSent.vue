<template>
  ...
  <div id="sent-list">
    <div class="admin-info-content">
      ...
      <tr v-for="item in (this.$store.state.ios_adminAppUserData)" :key="item.info[0].uid" class="list" :class="{active:item.info[0].reward_current == 0}">
  ...
  methods: {
    ...
    async updateIsSent(Id, respondedSurvey) {
      ...
      for(var i=0 ; i < rs.length ; i++) {
        console.log(rs[i])
        var itemRef = doc(db, "iOSTestPannelData", Id, "UserSurveyList", rs[i].toString())
    ...
    async clearCurrent(Id, reward_current){
      const db = this.$store.state.db
      const rewardRef = doc(db, "iOSTestPannelData", Id)
    ...
    async appendAllList(){
      ...
      var lst = this.$store.state.ios_adminAppUserData
      if(this.check){
